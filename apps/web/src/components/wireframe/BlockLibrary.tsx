    ...
    <div className="w-44 shrink-0 bg-gray-950 border-r border-gray-800 flex flex-col">
          ...
          >
            <span
              className={`w-6 h-6 flex items-center justify-center rounded text-[11px] font-bold shrink-0 transition-colors
                ${disabled ? "bg-gray-800 text-gray-600" : "bg-gray-800 text-gray-300 group-hover:bg-brand-800/40 group-hover:text-brand-300"}`}
            >
              {icon}
            </span>
            <span className="text-xs text-gray-300 font-medium truncate">{type}</span>
          </div>
        ))}
      </div>
      ...
      {!disabled && (
        <div className="px-3 py-2 border-t border-gray-800">
          <p className="text-[10px] text-gray-600">Drag onto canvas to add</p>
        </div>
      )}
    </div>
  );
